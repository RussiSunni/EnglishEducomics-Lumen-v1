@extends('layout')


@section('content')


<div class="content">
    <div class="container">
        <div class="row justify-content-center mt-3">
            <img src="../../images/a-trip-to-victorian-england/present-simple-tense-title.svg" class="img-fluid">
        </div>
        <div class="row mt-3">
            <div class="col-md-5">
                <img src="../../images/a-trip-to-victorian-england/present-simple-tense-02.png" class="img-fluid rounded panel">
            </div>
            <div class="col-md-2 d-flex align-items-center justify-content-center mt-3 mb-3">
                    <img src="../../images/a-trip-to-victorian-england/for-example.svg">
            </div>
            <div class="col-md-5">
                <img src="../../images/a-trip-to-victorian-england/present-simple-tense-03.png" class="img-fluid rounded panel">
            </div>
        </div>
        <div class="row mt-2">
            <a class="btn btn-outline-dark mx-auto" href="07" role="button">next</a>
        </div>
    </div>
</div>

@endsection
